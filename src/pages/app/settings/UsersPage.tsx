...
export default function UsersPage() {
  const { data: apiData, loading, refetch } = useApi<any>("/api/users");
  const [search, setSearch] = useState("");
  const [showMatrix, setShowMatrix] = useState(false);

  const apiUsers: any[] = apiData?.data ?? [];

  const users: TeamUser[] = apiUsers.map((u: any) => ({
    id:        u.id,
    name:      `${u.firstName ?? ""} ${u.lastName ?? ""}`.trim() || u.email,
    email:     u.email,
    role:      (u.role as string).toLowerCase() as Role,
    status:    u.isVerified ? "active" as UserStatus : "invited" as UserStatus,
    lastLogin: u.lastLoginAt,
    avatar:    `${(u.firstName ?? "")[0] ?? ""}${(u.lastName ?? "")[0] ?? ""}`.toUpperCase() || "?",
    mfa:       u.isMfaEnabled ?? false,
  }));
  ...
  const handleInvite = () => toast({ title: "Bientôt disponible", description: "L'invitation d'utilisateurs sera disponible prochainement." });
  ...
  const handleDelete = async (user: TeamUser) => {
    ...
    try {
      ...
      if (res.ok) {
        toast({ title: "Utilisateur supprimé", description: `${user.name} a été retiré de l'organisation.` });
        refetch();
      } else {
    ...
    <motion.div className="p-6 space-y-6 max-w-4xl" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Button size="sm" className="gradient-primary text-primary-foreground" onClick={handleInvite}>
          ...
          {users.some(u => u.status === "active" && !u.mfa) && (
            <div className="flex items-center gap-3 p-3 rounded-lg border border-warning/30 bg-warning/5 text-sm">
              ...
              <Button size="sm" variant="outline" className="text-xs h-7 border-warning/40 text-warning flex-shrink-0"
                onClick={() => toast({ title: "Bientôt disponible", description: "Le forçage du 2FA sera disponible prochainement." })}>
                Forcer le 2FA
              </Button>
            </div>
          )}
            ...
            <CardContent className="p-0">
              {filtered.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground">{search ? "Aucun utilisateur trouvé" : "Aucun utilisateur dans l'organisation"}</p>
                </div>
              ) : (
                          ...
                          <td className="p-3 text-right">
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="text-xs">
                                <DropdownMenuItem onClick={() => toast({ title: "Bientôt disponible", description: "La modification de rôle sera disponible prochainement." })}>
                                  <Edit className="h-3 w-3 mr-2" />Modifier le rôle
                                </DropdownMenuItem>
                                {user.status === "invited" && (
                                  <DropdownMenuItem onClick={() => toast({ title: "Invitation renvoyée", description: `Un email a été envoyé à ${user.email}.` })}>
                                    <RefreshCw className="h-3 w-3 mr-2" />Renvoyer l'invitation
                                  </DropdownMenuItem>
                                )}
                                {user.role !== "owner" && (
